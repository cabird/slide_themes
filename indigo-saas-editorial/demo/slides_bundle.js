...
function sectionLabel(text, barId, labelId, x, y) {
  return [
    J(`<div style="width:100%;height:100%;background:${C.accentBlue};"></div>`, {
      id: barId,
      x,
      y,
      w: 4,
    ...
    J(`<p style="font-family:${FONT};font-size:16px;font-weight:500;color:${C.textPrimary};line-height:22px;">${text}</p>`, {
      id: labelId,
      x: x + 16,
      y,
      w: 300,
...
async function run() {
  ...
  const slides = [
    ...
    // ================================================================
    {
      ...
      elements: [
        // Logo
        J(`<p style="font-family:${FONT};font-size:18px;font-weight:500;color:${C.textOnDark};">CodeForge AI</p>`, {
          id: "s1-logo",
          x: safe.x,
          y: safe.y,
          w: 200,
        ...
        // URL top-right
        J(`<p style="font-family:${FONT};font-size:14px;font-weight:400;color:rgba(255,255,255,0.5);text-align:right;">codeforge.ai</p>`, {
          id: "s1-url",
          x: safe.x + safe.w - 200,
          y: safe.y,
          w: 200,
        ...
        // Hero headline
        J(`<h1 style="font-family:${FONT};font-size:72px;font-weight:700;color:${C.textOnDark};line-height:1.1;letter-spacing:-0.02em;">Ship Better Code,<br>Faster Than Ever</h1>`, {
          id: "s1-title",
          x: safe.x,
          y: safe.y + 170,
          w: 720,
        ...
        // Subtext
        J(`<p style="font-family:${FONT};font-size:20px;font-weight:400;color:rgba(255,255,255,0.7);line-height:1.6;">AI-powered code review, documentation, and quality assurance \u2014 built for engineering teams that move fast.</p>`, {
          id: "s1-body",
          x: safe.x,
          y: Ne("s1-title", { gap: 28 }),
          w: 600
        }),
        // Layered screenshot placeholders (right side)
        J(screenshotPlaceholder("Code Review Dashboard"), {
          id: "s1-screenshot1",
          x: 920,
          y: safe.y + 100,
          w: 780,
        ...
        J(screenshotPlaceholder("PR Analysis View"), {
          id: "s1-screenshot2",
          x: 1020,
          y: safe.y + 300,
          w: 680,
    ...
    // ================================================================
    {
      ...
      elements: (() => {
        const { left, right } = en(safe, { ratio: 0.55, gap: 60 });
        return [
          // Section label
          ...sectionLabel("Our Forte", "s2-bar", "s2-label", left.x, safe.y),
          // Headline
          J(`<h2 style="font-family:${FONT};font-size:44px;font-weight:700;color:${C.textPrimary};line-height:1.15;letter-spacing:-0.01em;">AI That Understands<br>Your Codebase</h2>`, {
            id: "s2-title",
            x: left.x,
            y: Ne("s2-label", { gap: 16 }),
            w: left.w,
            h: 110
          }),
          // Body text
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">CodeForge analyzes your entire repository to deliver context-aware reviews that catch real issues, not just style nits.</p>`, {
            id: "s2-body",
            x: left.x,
            y: Ne("s2-title", { gap: 20 }),
            w: left.w
          }),
          // Feature card 1
          J(`<div style="display:flex;align-items:flex-start;gap:16px;">
            ${iconCircle("\u26A1")}
            <div>
          ...
          J(`<div style="display:flex;align-items:flex-start;gap:16px;">
            ...
            id: "s2-feat3",
            x: left.x,
            y: Ne("s2-feat2", { gap: 28 }),
            w: left.w
          }),
          // Screenshot (right column)
          J(screenshotPlaceholder("Code Review UI"), {
            id: "s2-screenshot",
            x: right.x,
            y: right.y + 40,
            w: right.w,
    ...
    // ================================================================
    {
      ...
      elements: (() => {
        ...
        const elements = [
          ...
          J(`<h2 style="font-family:${FONT};font-size:44px;font-weight:700;color:${C.textPrimary};line-height:1.15;letter-spacing:-0.01em;">The CodeForge Way</h2>`, {
            id: "s3-title",
            x: safe.x,
            y: Ne("s3-label", { gap: 16 }),
            w: 700,
            h: 56
          }),
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">From setup to shipping, we integrate seamlessly into your workflow.</p>`, {
            id: "s3-body",
            x: safe.x,
            y: Ne("s3-title", { gap: 16 }),
            w: 650
          })
        ];
        elements.push(
          J(`<div style="width:100%;height:2px;background:${C.border};"></div>`, {
            id: "s3-line",
            x: safe.x + cardW / 2,
            y: cardsY - 40,
            w: safe.w - cardW,
        ...
        for (let i = 0; i < steps.length; i++) {
          ...
          elements.push(
            J(`<div style="width:14px;height:14px;border-radius:50%;background:${C.accentYellow};border:3px solid ${C.bgPrimary};"></div>`, {
              id: `s3-dot${i}`,
              x: cx + cardW / 2 - 7,
              y: cardsY - 47,
              w: 14,
    ...
    // ================================================================
    {
      ...
      elements: (() => {
        const { left, right } = en(safe, { ratio: 0.35, gap: 50 });
        const languages = [
          { flag: "\u{1F1FA}\u{1F1F8}", lang: "English", sample: '"Refactor the auth middleware to support JWT rotation."' },
          { flag: "\u{1F1EF}\u{1F1F5}", lang: "\u65E5\u672C\u8A9E", sample: '"\u8A8D\u8A3C\u30DF\u30C9\u30EB\u30A6\u30A7\u30A2\u3092\u30EA\u30D5\u30A1\u30AF\u30BF\u30EA\u30F3\u30B0\u3057\u3066 JWT \u30ED\u30FC\u30C6\u30FC\u30B7\u30E7\u30F3\u306B\u5BFE\u5FDC\u3002"' },
          { flag: "\u{1F1EB}\u{1F1F7}", lang: "Fran\xE7ais", sample: `"Refactorer le middleware d'authentification pour le rotation JWT."` },
          { flag: "\u{1F1E8}\u{1F1F3}", lang: "\u4E2D\u6587", sample: '"\u91CD\u6784\u8BA4\u8BC1\u4E2D\u95F4\u4EF6\u4EE5\u652F\u6301 JWT \u8F6E\u6362\u3002"' }
        ];
        const elements = [
          // Avatar placeholder (left)
          J(`<div style="width:100%;height:100%;background:linear-gradient(135deg, #E8ECFF 0%, #F5F0FF 100%);border-radius:12px;display:flex;align-items:center;justify-content:center;">
            <div style="width:200px;height:200px;border-radius:50%;background:linear-gradient(135deg, ${C.accentBlue}22, ${C.tagPurple}22);border:2px solid ${C.accentBlue}33;display:flex;align-items:center;justify-content:center;">
              <span style="font-size:72px;line-height:1;">\u{1F916}</span>
            </div>
          </div>`, {
            id: "s4-avatar",
            x: left.x,
          ...
          // Headline (right)
          J(`<h2 style="font-family:${FONT};font-size:40px;font-weight:700;color:${C.textPrimary};line-height:1.2;letter-spacing:-0.01em;">AI Assistants That Speak Your Team's Language</h2>`, {
            id: "s4-title",
            x: right.x,
            y: right.y + 60,
            w: right.w,
          ...
          // Body text
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">CodeForge's review comments are generated in your team's native language, making feedback accessible to every developer worldwide.</p>`, {
            id: "s4-body",
            x: right.x,
            y: Ne("s4-title", { gap: 20 }),
            w: right.w
          })
        ];
        ...
        for (let i = 0; i < languages.length; i++) {
          const l = languages[i];
          elements.push(
            J(`<div style="display:flex;align-items:center;gap:14px;padding:10px 0;${i < languages.length - 1 ? `border-bottom:1px solid ${C.border};` : ""}">
    ...
    // ================================================================
    {
      ...
      elements: (() => {
        const elements = [
          ...
          J(`<h2 style="font-family:${FONT};font-size:44px;font-weight:700;color:${C.textPrimary};line-height:1.15;letter-spacing:-0.01em;">Your Workflow, Our Expertise:<br>A Perfect Fit</h2>`, {
            id: "s5-title",
            x: safe.x,
            y: Ne("s5-label", { gap: 16 }),
            w: 800,
            h: 110
          }),
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">Integrates natively with your existing tools \u2014 GitHub, GitLab, Bitbucket, Jira, Slack, and 40+ more platforms.</p>`, {
            id: "s5-body",
            x: safe.x,
            y: Ne("s5-title", { gap: 16 }),
            w: 750
          }),
          // Monitor mockup
          J(`<div style="width:100%;height:100%;background:#1A1A2E;border-radius:12px 12px 0 0;padding:12px;box-sizing:border-box;">
          ...
          // Monitor stand
          J(`<div style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;">
    ...
    // ================================================================
    {
      ...
      elements: (() => {
        ...
        return [
          ...
          J(`<h2 style="font-family:${FONT};font-size:44px;font-weight:700;color:${C.textPrimary};line-height:1.15;letter-spacing:-0.01em;">Your Partner in<br><strong style="color:${C.accentBlue};">Engineering Growth</strong></h2>`, {
            id: "s6-title",
            x: left.x,
            y: Ne("s6-label", { gap: 16 }),
            w: left.w,
            h: 110
          }),
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">We help engineering teams scale their review capacity without scaling headcount. CodeForge acts as an always-on senior engineer for every pull request.</p>`, {
            id: "s6-body",
            x: left.x,
            y: Ne("s6-title", { gap: 20 }),
            w: left.w
          }),
          // Sub-section 1
          J(`<div>
          ...
          // Small decorative screenshot
          J(screenshotPlaceholder("Growth Metrics"), {
            id: "s6-screenshot",
            x: right.x + 40,
            y: Ne("s6-sub2", { gap: 40 }),
            w: right.w - 80,
    ...
    // ================================================================
    {
      ...
      elements: (() => {
        ...
        const elements = [
          ...
          J(`<h2 style="font-family:${FONT};font-size:44px;font-weight:700;color:${C.textPrimary};line-height:1.15;letter-spacing:-0.01em;">CodeForge's Impact on<br>Your Dev Pipeline</h2>`, {
            id: "s7-title",
            x: safe.x,
            y: Ne("s7-label", { gap: 16 }),
            w: 700,
            h: 110
          }),
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">Every stage of your development workflow is enhanced with AI-powered analysis and automation.</p>`, {
            id: "s7-body",
            x: safe.x,
          ...
          J(`<div style="display:flex;gap:12px;flex-wrap:wrap;">
        ...
        for (let i = 0; i < funnelStages.length; i++) {
          ...
          elements.push(
            J(`<div style="display:flex;align-items:center;gap:12px;">
        ...
        elements.push(
          J(`<p style="font-family:${FONT};font-size:13px;font-weight:400;color:${C.textMuted};text-transform:uppercase;letter-spacing:1px;">End-to-End CI/CD Pipeline Coverage</p>`, {
            id: "s7-annotation",
            x: safe.x,
            y: safe.y + safe.h - 70,
            w: 400,
    ...
    // ================================================================
    {
      ...
      elements: (() => {
        ...
        return [
          ...
          J(`<h2 style="font-family:${FONT};font-size:44px;font-weight:700;color:${C.textPrimary};line-height:1.15;letter-spacing:-0.01em;">What Sets Us Apart</h2>`, {
            id: "s8-title",
            x: left.x,
            y: Ne("s8-label", { gap: 16 }),
            w: left.w,
            h: 56
          }),
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">Unlike generic AI code tools, CodeForge doesn't just pattern-match \u2014 it truly understands your architecture, conventions, and business logic.</p>`, {
            id: "s8-body1",
            x: left.x,
            y: Ne("s8-title", { gap: 20 }),
            w: left.w
          }),
          J(`<p style="font-family:${FONT};font-size:18px;font-weight:400;color:${C.textSecondary};line-height:1.6;">Our proprietary <strong style="color:${C.textPrimary};">Context Graph Engine</strong> maps every function, class, and dependency in your codebase, enabling reviews that consider ripple effects across the entire system.</p>`, {
            id: "s8-body2",
            x: left.x,
            y: Ne("s8-body1", { gap: 16 }),
            w: left.w
          }),
          ...
          J(`<div>
            ...
            id: "s8-checklist",
            x: left.x,
            y: Ne("s8-body2", { gap: 20 }),
            w: left.w
          }),
          // Screenshot (right column) — product visual / video player
          J(`<div style="width:100%;height:100%;background:#F0F2F8;border:1px solid ${C.border};border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,0.08);display:flex;flex-direction:column;">
